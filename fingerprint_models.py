...
"""
Define the StegaStamp Encoder and Decoder.
- StegaStampEncoder: Takes an input image and a fingerprint, then embeds the fingerprint into the image using a 
                     series of convolutional and upsampling layers.
                     The output is a watermarked image, optionally with the residuals from the embedding process.
- StegaStampDecoder: Takes the watermarked image as input and attempts to extract the embedded fingerprint using 
                     a series of convolutional layers followed by a fully connected network.
                     The decoder outputs the extracted fingerprint, which can be compared to the original for verification.
"""
